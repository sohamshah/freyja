...
class AgentType:
    ...
    # ─── Top-level preset surface ─────────────────────────────────────
    # When `top_level` is True, this AgentType is ALSO usable as the
    # PARENT session's preset — not just as a `sub_agent(agent_type=...)`
    # spawn target. The unified agent picker in the renderer shows
    # every top_level type as a selectable preset card alongside the
    # raw model list. Default False keeps every existing sub-agent
    # profile unchanged.

    top_level: bool = False
    """Whether this profile can be selected as the parent session's
    starting preset. False = subagent-only (existing behavior)."""

    nickname: str | None = None
    """Optional short label for the picker subtitle (e.g. "Claude Code"
    for an entry whose canonical name is `claude-code`). Falls back to
    the canonical name when unset."""

    coordination_strategy: str | None = None
    """Preferred coordination strategy when this preset is applied at
    session start. None = leave the operator's current strategy
    untouched. Only consulted when `top_level` is True."""
...
# ─── Top-level preset prompts ────────────────────────────────────────
#
# These prompts are SESSION-LEVEL — they describe what the parent agent
# should act like for an entire conversation, not what a one-shot sub
# agent does on a single delegated task. Each preset's prompt slots in
# at the top of the composed system prompt (after the date block) so
...
Operating principles
  · Read before you write. Use grep / glob / list_directory to ground
    yourself in the actual codebase before proposing changes. Don't
    invent file paths, function names, or APIs. If you reference
    something, it must exist on disk and you must have read it.
  · Prefer small, reviewable diffs over sweeping rewrites. When you
    edit, edit the minimum needed; surrounding code stays put.
  · Show, don't summarize. When you find something important, quote
    the file path and line numbers (e.g. `bridge/freyja_bridge.py:1234`)
    so the operator can navigate directly.
  · Run before you claim. Use bash to run linters, type-checks, or
    tests when verifying your work. Cite the actual command output.
  · Use kanban for decomposition — meaningful deliverables become
    cards, and workers pick them up. Use tasks for personal scratch
    inside one card.
  · When stuck, name the blocker concretely. Don't guess.
"""
...
Operating principles
  · Ground yourself in the codebase first — grep, glob, read the
    relevant files before proposing changes. No fabricated paths.
  · Bias toward direct execution. When the task is clear, take action
    and report what you did with concrete artifacts (file paths,
    diffs, test output) rather than asking for clarification first.
  · Diff-tight. Keep edits minimal. The operator reviews diffs, so
    sweeping reformatting attached to a real fix is noise.
  · Verify what you ship. Use bash for type checks / tests / lints
    and quote the output.
  · Use kanban for cross-agent deliverables; tasks for your own
    multi-step checklist within one card.
"""
...
Operating principles
  · Always take a screenshot before acting. The screen state can have
    changed since the last observation; don't trust stale beliefs.
  · One action per turn. Click → screenshot → look → next action. Do
    not stack three clicks before observing the result.
  · Read the accessibility tree when DOM-like structure exists (web
    pages, native UI). Coordinates are a last resort, not a first
    move.
  · Quote what you see. When you assert "the form has three fields,"
    refer to a specific screenshot or accessibility tree dump.
  · Never click "Allow" / "OK" / "Submit" on a permission or
    confirmation dialog without the operator's explicit go-ahead.
  · If you can't make progress in three actions, ask the operator
...
Operating principles
  · Decompose by question, not by source. Each spawnable subagent
    should own a sharp question, not "go look at the web for me."
  · Publish findings to the bus so siblings can read them. When you
    discover something useful, name it explicitly with the source.
  · Cite everything. URLs, file paths, line numbers, dates of pages
    you read. The synthesis is only useful if the operator can verify.
  · Watch for contradictions across sources. Surface them rather
    than smoothing them over.
  · Conclude with a structured synthesis: what's confirmed, what's
    contested, what's still unknown.
"""
...
Operating principles
  ...
  · Voice is intentional. Match the register the operator establishes
    — formal, technical, plainspoken, whatever it is. Don't drift
    into AI-default neutral.
  · Cite sources when making factual claims, even in narrative prose.
    Anchor opinions to evidence the operator can trace.
  · Revise on request. Read the operator's edit suggestions
    literally; don't second-guess the direction.
  · Use bash / web tools sparingly — only when you genuinely need a
    fact you don't have. Most of the work is at the keyboard.
"""
...
Operating principles
  ...
  · One paragraph or one short list. If the operator wants more,
    they'll ask.
  · Tools are available but expensive — use one only when the
    answer genuinely requires it.
  · Skip the "would you like me to…" closers. Just stop.
"""
...
AGENT_TYPES: dict[str, AgentType] = {
    ...
    # ─── Top-level session presets ────────────────────────────────────
    # These are the bundles exposed in the unified agent picker. Each
    # one is a complete starting configuration: model + thinking +
    # coordination strategy + tool surface + system prompt. The
    # operator can still tweak any field after applying; the preset
    # just sets a coherent starting point so they don't have to
    # configure six things to get a working coding agent.
    "claude-code": AgentType(
        name="claude-code",
        nickname="Claude Code",
        description="Anthropic coding agent: file-heavy, careful diffs, kanban handoffs.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        coordination_strategy="kanban",
        model="claude-opus-4-7",
        thinking_effort="high",
        model_policy="first_available",
        model_fallbacks=("claude-sonnet-4-6", "claude-opus-4-6"),
        tool_include=frozenset({
            "read_file", "write_file", "edit_file", "list_directory",
            "glob", "grep", "bash", "web_search", "web_fetch",
            "sub_agent", "subagents", "kanban", "tasks", "talk",
            "list_agent_sessions", "read_widget_spec", "show_widget",
            "artifacts", "publish_finding", "read_findings",
            "summarize_context", "record_user_preference",
            "list_skills", "search_skills", "load_skill",
        }),
        system_prompt=_CLAUDE_CODE_PRESET_PROMPT,
        max_iterations=iteration_cap("claude-code", 200),
    ),
    "codex": AgentType(
        name="codex",
        nickname="Codex",
        description="OpenAI coding agent: direct execution, diff-tight, GPT-5.5 brain.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        coordination_strategy="kanban",
        model="gpt-5.5",
        thinking_effort="high",
        model_policy="first_available",
        model_fallbacks=("gpt-5.4", "claude-opus-4-7"),
        tool_include=frozenset({
            "read_file", "write_file", "edit_file", "list_directory",
            "glob", "grep", "bash", "web_search", "web_fetch",
            "sub_agent", "subagents", "kanban", "tasks", "talk",
        ...
        system_prompt=_CODEX_PRESET_PROMPT,
        max_iterations=iteration_cap("codex", 200),
    ),
    "computer-use": AgentType(
        name="computer-use",
        nickname="Computer Use",
        description="Screen-driven agent: screenshots, click/type, accessibility-tree reads.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        coordination_strategy="isolated",
        model="claude-opus-4-7",
        thinking_effort="high",
        model_policy="first_available",
        model_fallbacks=("gpt-5.5", "claude-sonnet-4-6"),
        tool_include=frozenset({
            "computer_screenshot", "computer_click", "computer_type",
            "computer_scroll", "computer_key", "computer_move_mouse",
            "computer_inspect", "computer_focus_window", "computer_list_windows",
            "computer_read_ax_tree", "computer_find_element",
            "browser_screenshot", "browser_execute_js",
            "read_file", "list_directory", "bash",
            "tasks", "artifacts",
        }),
        system_prompt=_COMPUTER_USE_PRESET_PROMPT,
        max_iterations=iteration_cap("computer-use", 300),
    ),
    "research-assistant": AgentType(
        name="research-assistant",
        nickname="Research Assistant",
        description="Multi-agent research: bus-coordinated fanout with cited synthesis.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        ...
        thinking_effort="medium",
        model_policy="first_available",
        model_fallbacks=("claude-opus-4-7", "gpt-5.5", "kimi-k2.6"),
        tool_include=frozenset({
            "web_search", "web_fetch",
            "read_file", "write_file", "list_directory", "glob", "grep",
            "bash",
            "sub_agent", "subagents", "talk", "list_agent_sessions",
            "publish_finding", "read_findings",
            "tasks", "artifacts",
            "summarize_context", "record_user_preference",
            "list_skills", "search_skills", "load_skill",
            "analyze_video",
        }),
        system_prompt=_RESEARCH_ASSISTANT_PRESET_PROMPT,
        max_iterations=iteration_cap("research-assistant", 160),
    ),
    "long-form-writer": AgentType(
        name="long-form-writer",
        nickname="Writer",
        description="Prose-focused work: outlines, drafts, revision. Minimal tools.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        coordination_strategy="bus",
        ...
        tool_include=frozenset({
            "read_file", "write_file", "list_directory", "glob",
            "web_search", "web_fetch",
            "tasks", "artifacts",
            "summarize_context", "record_user_preference",
            "list_skills", "search_skills", "load_skill",
        }),
        system_prompt=_LONG_FORM_WRITER_PRESET_PROMPT,
        max_iterations=iteration_cap("long-form-writer", 80),
    ),
    "quick-chat": AgentType(
        name="quick-chat",
        nickname="Quick Chat",
        description="Short, direct answers. Haiku-class model, minimal tool surface.",
        usage_hint="(top-level preset; not used for spawning subagents)",
        top_level=True,
        coordination_strategy="bus",
        ...
        thinking_effort="off",
        model_policy="first_available",
        model_fallbacks=("gpt-5.4-mini", "kimi-k2.5"),
        tool_include=frozenset({
            "read_file", "list_directory", "glob",
            "web_search", "web_fetch",
        }),
        system_prompt=_QUICK_CHAT_PRESET_PROMPT,
        max_iterations=iteration_cap("quick-chat", 40),
    ),
}
