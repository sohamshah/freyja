...
def coordination_prompt(value: str | None) -> str:
    ...
    if strategy == STRATEGY_KANBAN:
        return (
            ...
            "  The board exists so workers can pick cards up. If you create a card\n"
            "  and then immediately start executing its work in your own session,\n"
            "  you are acting as both decomposer and worker — defeating the point\n"
            "  of the board. If autopilot is off and you need a card executed now,\n"
            "  tell the operator that the board is queued and autopilot is\n"
            "  disabled. Do not silently become the worker.\n"
            "\n"
            ...
            "    untracked. Decomposition belongs on the board.\n"
