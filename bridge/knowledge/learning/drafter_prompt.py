...
AGENTIC_FREYJA_CONTEXT = (
    ...
    "Runtime contract:\n"
    "  · You have tools. Use them. The list at the top of this prompt is "
    "your full toolset — read tools (``read_file``, ``list_directory``, "
    "``glob``, ``grep``, ``bash`` for read-only inspection), skill-"
    "library tools (``list_skills``, ``search_skills``, ``load_skill``), "
    "and one publish tool (``propose_skill``).\n"
